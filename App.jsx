...
import { 
  ...
  Keyboard, 
  Languages,
  Wand2,          // New icon for AI generation
  BrainCircuit,   // New icon for AI analysis
  Loader2         // New icon for loading state
} from 'lucide-react';

// ==========================================
// 1. CONSTANTS & DATA & INITIALIZATION
// ==========================================

const STORAGE_KEYS = {
    GOALS: 'ielts_pro_v2_goals',
    BIRTH_DATE: 'ielts_pro_v2_birth_date',
    TOTAL_TIME: 'ielts_pro_v2_total_time',
    ACTIVE_USERS: 'ielts_pro_v2_active_users',
    HOMEWORK: 'ielts_pro_v2_homework_calendar',
    TRACKER_ROWS: 'ielts_pro_v2_tracker_rows',
    TRACKER_DATA: 'ielts_pro_v2_tracker_data',
    // New Content Storage Keys
    DATA_GRAMMAR: 'ielts_pro_v2_data_grammar',
    DATA_READING: 'ielts_pro_v2_data_reading',
    DATA_WRITING: 'ielts_pro_v2_data_writing',
    DATA_SPEAKING: 'ielts_pro_v2_data_speaking',
    DATA_VOCABULARY: 'ielts_pro_v2_data_vocabulary',
    GEMINI_KEY: 'ielts_pro_gemini_api_key' // Key for Gemini API
};
...
// --- STATIC DATA FOR INITIALIZATION (OLD DATA) ---
const PDF_TEXTS = {
    ...
    THERE_IS: `THERE IS / THERE ARE\n\nBorlikni bildiradi.\nBirlik: There is a book.\nKo'plik: There are books.`,
...
const formatCountdown = (diff) => {
    ...
    return `${days} kun ${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`;
};
...
const getInitialGrammarData = () => {
    const saved = localStorage.getItem(STORAGE_KEYS.DATA_GRAMMAR);
    if (saved) return JSON.parse(saved);

    return GRAMMAR_STRUCTURE_RAW.map(cat => ({
        id: 'cat_' + Math.random().toString(36).substr(2, 9),
        title: cat.category,
        topics: cat.topics.map(topicTitle => ({
            id: 'top_' + Math.random().toString(36).substr(2, 9),
            title: topicTitle,
            content: GRAMMAR_CONTENT_MAP[topicTitle] || "" 
        }))
    }));
};
...
const callGemini = async (prompt) => {
    ...
    if (!key) throw new Error("API Kaliti topilmadi. Iltimos, Sozlamalar bo'limidan Google Gemini API kalitini kiriting.");
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(err.error?.message || "Xatolik yuz berdi");
        }
...
const Button = ({ children, onClick, variant = "primary", className = "", disabled = false, icon: Icon }) => {
  const variants = {
    ...
    ai: "bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white hover:from-violet-700 hover:to-fuchsia-700 shadow-lg shadow-purple-200", // New AI variant
    ...
    return (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                <h3 className="text-lg font-bold text-slate-900 mb-2 flex items-center gap-2"><AlertTriangle className="text-rose-500"/> Tasdiqlash</h3>
...
const AIGeneratorModal = ({ onClose, onSave }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const prompt = `Generate a list of 10 advanced IELTS vocabulary words related to the topic "${topic}". 
            Structure the output clearly. For each word include:
            1. The word (in bold)
            2. Definition (in English)
            3. Uzbek translation (in brackets)
            4. Example sentence.
    ...
    return (
        <div className="fixed inset-0 z-[250] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[85vh]">
                <div className="p-6 border-b flex justify-between items-center bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-t-2xl">
                    <h3 className="text-xl font-bold flex items-center gap-2"><Sparkles/> AI So'zlar Generatori</h3>
                    <button onClick={onClose} className="hover:bg-white/20 p-1 rounded-full transition-colors"><X size={24}/></button>
                </div>
                <div className="p-6 flex-1 overflow-y-auto">
                    {!result ? (
                        <div className="space-y-4">
                            <p className="text-slate-600">Qaysi mavzuda so'zlar o'rganmoqchisiz? (Masalan: Environment, Technology, Education)</p>
                            <div className="flex gap-2">
                                <input 
                                    value={topic} 
                                    onChange={e=>setTopic(e.target.value)} 
                                    placeholder="Mavzuni kiriting..." 
                                    className="flex-1 border-2 border-slate-200 rounded-xl px-4 py-3 text-lg focus:border-purple-500 focus:outline-none transition-colors"
                                    onKeyDown={e=>e.key==='Enter' && handleGenerate()}
                                />
                                <Button onClick={handleGenerate} variant="ai" disabled={loading} icon={Wand2}>
                                    {loading ? "Yaratilmoqda..." : "Yaratish"}
                                </Button>
                            </div>
                            {loading && (
                                <div className="flex flex-col items-center justify-center py-10 space-y-4 text-purple-600">
                                    <Loader2 size={48} className="animate-spin"/>
                                    <p className="font-bold animate-pulse">AI o'ylamoqda...</p>
                                </div>
                            )}
                            {error && (
                        ...
                        <div className="space-y-4">
                            <div className="prose max-w-none bg-slate-50 p-6 rounded-xl border border-slate-200 whitespace-pre-wrap font-serif text-lg">
                                {result}
                            </div>
                            <div className="flex justify-end gap-3">
                                <Button onClick={()=>setResult(null)} variant="secondary" icon={RotateCcw}>Qayta</Button>
                                <Button onClick={handleSaveResult} variant="success" icon={Save}>Saqlash</Button>
                            </div>
                        </div>
                    )}
                </div>
...
// --- NEW: AI ANALYZER MODAL ---
const AIAnalyzerModal = ({ content, onClose }) => {
    ...
    useEffect(() => {
        const analyze = async () => {
            try {
                const prompt = `Act as an IELTS examiner. Analyze the following text. 
                Identify grammar errors, suggest better vocabulary, and estimate a band score (if applicable).
                Keep it concise, helpful and encouraging.
                Text: "${content.substring(0, 1000)}..."`; 
                const text = await callGemini(prompt);
                setResult(text);
            } catch (e) {
                setResult(`Xatolik: ${e.message}`);
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="fixed inset-0 z-[250] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[85vh]">
                <div className="p-4 border-b flex justify-between items-center bg-slate-50 rounded-t-2xl">
                    <h3 className="font-bold text-slate-700 flex items-center gap-2"><BrainCircuit className="text-purple-600"/> AI Tahlili</h3>
                ...
                <div className="p-6 overflow-y-auto">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-10 space-y-3 text-slate-400">
                            ...
                            <p>Matn tekshirilmoqda...</p>
                        </div>
                    ) : (
                        <div className="prose max-w-none text-slate-700 whitespace-pre-wrap font-serif leading-relaxed">
                            {result}
                        </div>
                    )}
                </div>
                <div className="p-4 border-t bg-slate-50 rounded-b-2xl text-right">
                    <Button onClick={onClose} variant="primary">Yopish</Button>
                </div>
            </div>
        </div>
    );
...
const ContentEditorModal = ({ topic, onSave, onClose }) => {
    ...
    useEffect(() => {
        if (!topic.title && !topic.content) setIsEditing(true); // New topic, start in edit mode
    }, []);

    const handleSave = () => {
        onSave(topic.id, title, content);
        setIsEditing(false);
    };
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-slate-50 animate-in slide-in-from-right duration-300 flex flex-col">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {isEditing ? (
                         <input 
                            ...
                            className="text-xl font-bold text-slate-900 border-b-2 border-indigo-500 focus:outline-none bg-transparent w-full max-w-md"
                            placeholder="Mavzu sarlavhasi"
                         />
                    ) : (
                        <h2 className="text-xl font-bold text-slate-900 truncate max-w-xs sm:max-w-md md:max-w-xl">{title}</h2>
                    )}
                </div>
                <div className="flex gap-2">
                    {/* AI Button - Show only if content exists */}
                    {content.length > 20 && (
                        <Button onClick={() => setShowAI(true)} variant="ai" icon={Sparkles} className="hidden sm:flex">
                            AI Tahlil
                        </Button>
                    )}
                    
                    {isEditing ? (
                        <Button onClick={handleSave} variant="success" icon={Save}>Saqlash</Button>
                    ) : (
                        <Button onClick={() => setIsEditing(true)} variant="secondary" icon={Edit2}>Tahrirlash</Button>
                    )}
                ...
                <div className="bg-white rounded-3xl p-8 lg:p-12 shadow-sm border border-slate-200">
                    {isEditing ? (
                        <textarea
                            ...
                            className="w-full h-[60vh] p-4 text-slate-700 leading-relaxed font-serif text-lg border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                            placeholder="Mavzu matnini bu yerga yozing..."
                        />
                    ) : (
                        <div className="prose prose-slate lg:prose-lg max-w-none">
                            <h1 className="text-3xl font-bold text-indigo-700 mb-6 border-b pb-4">{title}</h1>
                            <div className="text-slate-700 leading-relaxed whitespace-pre-wrap font-serif text-lg">
                                {content ? content : (
                                    <div className="flex flex-col items-center justify-center py-20 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                                        ...
                                        <p className="font-medium">Ma'lumot kiritilmagan. Tahrirlash tugmasini bosing.</p>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {showAI && <AIAnalyzerModal content={content} onClose={() => setShowAI(false)} />}
        </div>
    );
};

const CourseManager = ({ title, icon: Icon, data, onUpdate, allowAIGen = false }) => {
    ...
    const addCategory = () => {
        const newCat = { id: 'cat_' + Date.now(), title: "Yangi Bo'lim", topics: [] };
        onUpdate([...data, newCat]);
        setEditingCategory(newCat.id);
        setNewCatName("Yangi Bo'lim");
    };

    const updateCategoryName = (id) => {
        if (!newCatName.trim()) return;
        const newData = data.map(cat => cat.id === id ? { ...cat, title: newCatName } : cat);
        onUpdate(newData);
        setEditingCategory(null);
    };

    const requestDeleteCategory = (id) => {
        setDeleteInfo({ type: 'cat', id });
    };

    const confirmDelete = () => {
        if (!deleteInfo) return;
        if (deleteInfo.type === 'cat') {
            onUpdate(data.filter(cat => cat.id !== deleteInfo.id));
        } else if (deleteInfo.type === 'topic') {
            const newData = data.map(cat => {
                if (cat.id === deleteInfo.parentId) {
                    return { ...cat, topics: cat.topics.filter(t => t.id !== deleteInfo.id) };
                }
                return cat;
            });
            onUpdate(newData);
    ...
    const addTopic = (catId) => {
        const newTopic = { id: 'top_' + Date.now(), title: "Yangi Mavzu", content: "" };
        const newData = data.map(cat => {
            if (cat.id === catId) {
                return { ...cat, topics: [...cat.topics, newTopic] };
            }
            return cat;
        });
        onUpdate(newData);
    ...
    const handleAISave = (topicTitle, content) => {
        // Look for "AI Generated" category or create one
        let targetCat = data.find(c => c.title === "AI Generated");
        let newData = [...data];
        
        if (!targetCat) {
            targetCat = { id: 'cat_ai_' + Date.now(), title: "AI Generated", topics: [] };
            newData.push(targetCat);
        }
        
        const newTopic = { id: 'top_ai_' + Date.now(), title: topicTitle, content: content };
        
        const finalData = newData.map(c => {
            if (c.id === targetCat.id) {
                return { ...c, topics: [newTopic, ...c.topics] };
            }
            return c;
        });
    ...
    const requestDeleteTopic = (catId, topicId, e) => {
        e.stopPropagation();
        setDeleteInfo({ type: 'topic', id: topicId, parentId: catId });
    };

    const handleTopicSave = (topicId, newTitle, newContent) => {
        const newData = data.map(cat => ({
            ...cat,
            topics: cat.topics.map(t => t.id === topicId ? { ...t, title: newTitle, content: newContent } : t)
        }));
        onUpdate(newData);
    };
    ...
    return (
        <>
            <div className="h-full p-6 lg:p-10 max-w-5xl mx-auto flex flex-col overflow-hidden">
                <div className="mb-8 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-slate-500">Mavzularni boshqarish va o'rganish</p>
                    </div>
                    <div className="flex gap-2">
                        {allowAIGen && (
                            <Button onClick={() => setShowAIGen(true)} variant="ai" icon={Sparkles}>AI So'zlar</Button>
                        )}
                        <Button onClick={addCategory} icon={Plus} variant="primary">Bo'lim qo'shish</Button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-3 pb-20">
                    {data.length === 0 && (
                         <div className="flex flex-col items-center justify-center py-20 text-slate-400 bg-white rounded-3xl border border-dashed border-slate-300">
                             ...
                             <p className="font-medium text-lg">Hozircha bo'limlar yo'q</p>
                             <p className="text-sm">"Bo'lim qo'shish" tugmasi orqali boshlang</p>
                         </div>
                    )}
                    {data.map((group) => (
                        <div key={group.id} className="border border-slate-200 rounded-xl bg-white overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md">
                            <div className={`w-full px-6 py-4 flex items-center justify-between text-left font-bold text-slate-700 transition-colors ${expandedCategory === group.id ? 'bg-indigo-50 text-indigo-700' : ''}`}>
                                <div className="flex-1 flex items-center gap-2">
                                    <button onClick={() => setExpandedCategory(expandedCategory === group.id ? null : group.id)} className="mr-2">
                                         {expandedCategory === group.id ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
                                    </button>
                                    ...
                                    {editingCategory === group.id ? (
                                        <div className="flex items-center gap-2 w-full max-w-md">
                                            <input 
                                                ...
                                                onKeyDown={(e) => e.key === 'Enter' && updateCategoryName(group.id)}
                                            />
                                            <button onClick={() => updateCategoryName(group.id)} className="text-emerald-600 hover:bg-emerald-50 p-1 rounded"><Save size={16}/></button>
                                        </div>
                                    ) : (
                                        <span onClick={() => setExpandedCategory(expandedCategory === group.id ? null : group.id)} className="cursor-pointer select-none flex-1">{group.title}</span>
                                    )}
                                </div>
                                <div className="flex gap-1 ml-4">
                                     {editingCategory !== group.id && (
                                        <>
                                            <button onClick={() => { setEditingCategory(group.id); setNewCatName(group.title); }} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-100 rounded-lg transition-colors"><Edit2 size={16} /></button>
                                            <button onClick={() => requestDeleteCategory(group.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-100 rounded-lg transition-colors"><Trash2 size={16} /></button>
                                        </>
                                     )}
                                </div>
                            </div>
                                ...
                                <div className="bg-slate-50/50 border-t border-slate-100">
                                    {group.topics.map((topic) => (
                                        <div 
                                            ...
                                            className="px-6 py-3 pl-10 text-sm text-slate-600 border-b border-slate-100 last:border-0 hover:bg-white hover:text-indigo-600 cursor-pointer transition-colors flex items-center justify-between group/topic"
                                        >
                                            <div className="flex items-center gap-2">
                                                <div className="w-1.5 h-1.5 rounded-full bg-indigo-300"></div>
                                                {topic.title}
                                            </div>
                                            <div className="opacity-0 group-hover/topic:opacity-100 flex gap-2">
                                                <span className="text-xs text-indigo-400 font-medium">Tahrirlash</span>
                                                <button onClick={(e) => requestDeleteTopic(group.id, topic.id, e)} className="text-slate-400 hover:text-rose-500"><Trash2 size={14} /></button>
                                            </div>
                                        </div>
                                    ))}
                                    <div className="p-3 pl-10">
                                        <button onClick={() => addTopic(group.id)} className="text-xs font-bold text-indigo-600 flex items-center gap-1 hover:bg-indigo-50 px-3 py-2 rounded-lg transition-colors">
                                            <Plus size={14}/> Yangi mavzu qo'shish
                                        </button>
                                    </div>
                                </div>
                            )}
            ...
            {showAIGen && <AIGeneratorModal onClose={() => setShowAIGen(false)} onSave={handleAISave} />}

            <ConfirmModal 
                isOpen={!!deleteInfo}
                message={deleteInfo?.type === 'cat' ? "Bu bo'limni va uning ichidagi barcha mavzularni o'chirmoqchimisiz?" : "Mavzuni o'chirmoqchimisiz?"}
...
const MonthlyTracker = ({ selectedDate }) => {
    ...
    const addRow = () => {
        ...
        setRows([...rows, { id: Date.now(), title: newRowTitle }]);
        setNewRowTitle("");
    };
    ...
    return (
        <div className="mt-8 bg-[#1e293b] rounded-3xl p-6 shadow-lg overflow-hidden text-slate-200">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-white flex items-center gap-2"><Target className="text-emerald-400" /> Oylik Tracker</h3>
            </div>
                ...
                <div className="min-w-[800px]">
                    <div className="flex gap-1 mb-2">
                        ...
                        {Array.from({ length: daysInMonth }).map((_, i) => {
                             ...
                             return (
                                <div key={i} className={`flex-1 text-center text-xs font-bold w-8 flex-shrink-0 flex flex-col items-center gap-0.5
                                    ${new Date(year, month, i + 1).toDateString() === new Date().toDateString() ? 'text-emerald-400' : 'text-slate-500'}`}>
                                    <span>{i + 1}</span>
                                    <div className={`w-1 h-1 rounded-full ${isTargetDay ? 'bg-indigo-500' : 'bg-transparent'}`}></div>
                                </div>
                            );
                        })}
                    </div>
                    <div className="space-y-1">
                        {rows.map(row => (
                            <div key={row.id} className="flex gap-1 items-center group">
                                <div className="w-48 flex-shrink-0 flex justify-between items-center pr-4 text-sm font-medium text-slate-300 bg-slate-800/50 p-2 rounded-lg border border-slate-700/50">
                                    <span className="truncate">{row.title}</span>
                                    <button onClick={() => setRowToDelete(row.id)} className="opacity-0 group-hover:opacity-100 text-slate-500 hover:text-rose-500 transition-all">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                                {Array.from({ length: daysInMonth }).map((_, i) => {
                                    ...
                                    return (
                                        <div key={i} onClick={() => handleCellClick(row.id, day)} className={`flex-1 h-9 rounded-md flex items-center justify-center cursor-pointer transition-all border border-slate-700/30 text-sm font-bold select-none w-8 flex-shrink-0 ${bgClass} ${isToday ? 'ring-1 ring-emerald-500/50' : ''}`}>{content}</div>
                                    );
                                })}
                            </div>
                        ))}
                    ...
                    <div className="mt-4 flex gap-2 w-full max-w-xs">
                        <input type="text" value={newRowTitle} onChange={(e) => setNewRowTitle(e.target.value)} placeholder="Yangi odat qo'shish..." className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-emerald-500" />
                        <button onClick={addRow} className="bg-emerald-600 text-white p-2 rounded-lg hover:bg-emerald-700 transition-colors"><Plus size={18} /></button>
                    </div>
                </div>
            </div>
            <ConfirmModal 
                isOpen={!!rowToDelete} 
                message="Bu qatorni o'chirmoqchimisiz?" 
...
const HomeworkModal = ({ onClose }) => {
    ...
    const handleToggleTask = (id) => {
        const isCompleting = !tasksForDate.find(t => t.id === id)?.completed;
        if (isCompleting) { setShowConfetti(true); setTimeout(() => setShowConfetti(false), 2500); }
        setAllTasks(prev => ({ ...prev, [selectedDate]: prev[selectedDate].map(t => t.id === id ? { ...t, completed: !t.completed } : t) }));
    };
    ...
    const saveEditing = () => {
        ...
        setAllTasks(prev => ({ ...prev, [selectedDate]: prev[selectedDate].map(t => t.id === editingTaskId ? { ...t, text: editingTaskText } : t) }));
        ...
        <div className="fixed inset-0 z-[100] bg-slate-50 flex flex-col animate-in fade-in duration-200">
            ...
            <div className="bg-white border-b border-slate-200 p-4 shadow-sm flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 transition-colors"><X size={24} /></button>
                    <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2"><FileText className="text-indigo-600" /> Uy Vazifalari & Tracker</h2>
                </div>
                <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-xl">
                    <button onClick={() => changeDate(-1)} className="p-2 hover:bg-white rounded-lg text-slate-600 shadow-sm transition-all"><ChevronLeft size={20} /></button>
                    <div className="flex flex-col items-center min-w-[160px] relative">
                        <span className="text-sm font-bold text-slate-800">{formatDateDisplay(selectedDate)}</span>
                        <input type="date" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} />
                    </div>
                    <button onClick={() => changeDate(1)} className="p-2 hover:bg-white rounded-lg text-slate-600 shadow-sm transition-all"><ChevronRight size={20} /></button>
                </div>
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-slate-200 min-h-[400px] flex flex-col mb-8">
                    <div className="p-6 border-b border-slate-100 flex flex-col gap-2">
                        <div className="flex justify-between items-center">
                            <h3 className="font-bold text-slate-800 text-lg">Kunlik Reja</h3>
                            <span className="text-xs font-bold text-indigo-500 bg-indigo-50 px-3 py-1 rounded-full">{totalCount} ta vazifa</span>
                        </div>
                        <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden"><div className="bg-emerald-500 h-full transition-all duration-500 ease-out" style={{ width: `${progress}%` }}></div></div>
                        <div className="flex justify-end"><span className="text-[10px] font-bold text-slate-400">{progress}% Bajarildi</span></div>
                    </div>
                    <div className="flex-1 p-6 space-y-3">
                        {tasksForDate.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-slate-400 py-10"><Calendar size={48} className="mb-4 text-slate-200" /><p className="text-lg font-medium">Vazifalar yo'q</p></div>
                        ) : (
                            tasksForDate.map(task => (
                                <div key={task.id} className={`group flex items-center gap-4 p-4 rounded-2xl border transition-all duration-200
                                     ${task.completed ? 'bg-emerald-50/50 border-emerald-100' : 'bg-white border-slate-100 hover:border-indigo-200 hover:shadow-md'}`}>
                                    <button onClick={() => handleToggleTask(task.id)} className={`flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${task.completed ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 hover:border-indigo-500'}`}>{task.completed && <CheckCircle2 size={16} className="text-white" />}</button>
                                    <div className="flex-1">
                                        {editingTaskId === task.id ? (
                                            <div className="flex gap-2">
                                                <input autoFocus type="text" value={editingTaskText} onChange={(e) => setEditingTaskText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && saveEditing()} className="flex-1 bg-white border border-indigo-300 rounded-lg px-3 py-1 text-sm focus:outline-none" />
                                                <button onClick={saveEditing} className="text-emerald-600 hover:bg-emerald-50 p-1 rounded"><Save size={18}/></button>
                                                <button onClick={() => setEditingTaskId(null)} className="text-rose-600 hover:bg-rose-50 p-1 rounded"><XCircle size={18}/></button>
                                            </div>
                                        ) : (
                                            <span className={`text-base font-medium transition-all ${task.completed ? 'text-emerald-700 line-through decoration-emerald-300' : 'text-slate-700'}`}>
                                                {task.text}
                                            </span>
                                        )}
                                    </div>
                                    {editingTaskId !== task.id && (
                                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => startEditing(task)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"><Edit2 size={18} /></button>
                                            <button onClick={() => handleRemoveTask(task.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"><Trash2 size={18} /></button>
                                        </div>
                                    )}
                                </div>
                            ))
                    ...
                    <div className="p-6 bg-slate-50 border-t border-slate-200 rounded-b-3xl">
                        <div className="flex gap-4 items-center bg-white p-2 rounded-2xl shadow-sm border border-slate-200 focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all">
                            <Plus size={24} className="text-slate-400 ml-2" />
                            <input type="text" value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddTask()} placeholder={`${formatDateDisplay(selectedDate)} uchun vazifa qo'shish...`} className="flex-1 py-3 bg-transparent text-slate-700 font-medium placeholder:text-slate-400 focus:outline-none" />
                            <Button onClick={handleAddTask} disabled={!newTaskText.trim()} className="h-10 px-6 shadow-none">Qo'shish</Button>
                        </div>
                    </div>
                </div>
...
const SettingsModal = ({ onClose }) => {
    const [apiKey, setApiKey] = useState(() => localStorage.getItem(STORAGE_KEYS.GEMINI_KEY) || "");

    const saveSettings = () => {
        localStorage.setItem(STORAGE_KEYS.GEMINI_KEY, apiKey);
        onClose();
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white p-6 rounded-2xl shadow-xl max-w-sm w-full mx-4" onClick={e => e.stopPropagation()}>
                <h3 className="text-xl font-bold mb-4 flex items-center gap-2"><Settings size={20}/> Sozlamalar</h3>
                
                <div className="mb-4">
                    <label className="block text-sm font-bold text-slate-700 mb-2">Google Gemini API Kaliti</label>
                    <input 
                        type="password"
                        value={apiKey}
                    ...
                    <p className="text-[10px] text-slate-500 mt-2">
                        API kalitni <a href="https://aistudio.google.com/app/apikey" target="_blank" className="text-indigo-600 underline">bu yerdan</a> olishingiz mumkin.
                    </p>
                </div>
      ...
      <div className="h-full p-6 lg:p-10 max-w-5xl mx-auto flex flex-col">
          ...
          <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
              ...
              <Card className="lg:col-span-8 p-8 flex flex-col items-center justify-center relative min-h-[400px]">
                  {['numbers_1_10', 'numbers_10_100', 'numbers_large', 'letters', 'letters_vowels'].includes(category) && (<div className="absolute top-6 left-6 flex items-center gap-2 bg-slate-50 p-1.5 rounded-lg border border-slate-100"><Timer size={16} className="text-slate-400 ml-1" />{[0, 2, 3, 5, 7].map(time => (<button key={time} onClick={() => { setTimerMode(time); if(time===0) setTimeLeft(0); }} className={`text-xs font-bold px-3 py-1.5 rounded-md transition-all ${timerMode === time ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-white hover:text-slate-700'}`}>{time === 0 ? 'OFF' : `${time}s`}</button>))}</div>)}
...
const Sidebar = ({ activeTab, setActiveTab, mobileOpen, setMobileOpen, onOpenSettings }) => {
  const menu = [ 
      ...
      { id: 'vocabulary', icon: Languages, label: 'Vocabulary' } 
  ];
  return (
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-[#0f172a] text-slate-300 flex flex-col transition-transform duration-300 border-r border-slate-800 shadow-2xl ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-4 mt-auto border-t border-slate-800">
             <button onClick={onOpenSettings} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium text-sm text-slate-400 hover:bg-slate-800 hover:text-white mb-2">
                <Settings size={18} />
            ...
            <a 
                ...
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium text-sm bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-white"
            >
                <Keyboard size={18} />
...
const App = () => {
  ...
  // -- Course Data States --
  const [grammarData, setGrammarData] = useState(getInitialGrammarData());
  const [readingData, setReadingData] = useState(() => getInitialData(STORAGE_KEYS.DATA_READING));
  const [writingData, setWritingData] = useState(() => getInitialData(STORAGE_KEYS.DATA_WRITING));
  const [speakingData, setSpeakingData] = useState(() => getInitialData(STORAGE_KEYS.DATA_SPEAKING));
  const [vocabularyData, setVocabularyData] = useState(() => getInitialData(STORAGE_KEYS.DATA_VOCABULARY));

  // -- Persistence Effects --
  useEffect(() => localStorage.setItem(STORAGE_KEYS.DATA_GRAMMAR, JSON.stringify(grammarData)), [grammarData]);
  useEffect(() => localStorage.setItem(STORAGE_KEYS.DATA_READING, JSON.stringify(readingData)), [readingData]);
  useEffect(() => localStorage.setItem(STORAGE_KEYS.DATA_WRITING, JSON.stringify(writingData)), [writingData]);
  useEffect(() => localStorage.setItem(STORAGE_KEYS.DATA_SPEAKING, JSON.stringify(speakingData)), [speakingData]);
  useEffect(() => localStorage.setItem(STORAGE_KEYS.DATA_VOCABULARY, JSON.stringify(vocabularyData)), [vocabularyData]);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'reading': return <CourseManager title="Reading Bo'limi" icon={BookOpen} data={readingData} onUpdate={setReadingData} />;
      case 'grammar': return <CourseManager title="Grammatika Mundarijasi" icon={GraduationCap} data={grammarData} onUpdate={setGrammarData} />;
      case 'writing': return <CourseManager title="Writing Bo'limi" icon={PenTool} data={writingData} onUpdate={setWritingData} />;
      case 'speaking': return <CourseManager title="Speaking Bo'limi" icon={Mic} data={speakingData} onUpdate={setSpeakingData} />;
      case 'vocabulary': return <CourseManager title="Vocabulary (Lug'at)" icon={Languages} data={vocabularyData} onUpdate={setVocabularyData} allowAIGen={true} />;
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} onOpenSettings={() => setShowSettingsModal(true)} />
